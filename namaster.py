...
pure_B = sparse.load_npz('pi_B_2962.npz')
...
f_mat = nmt.NmtField(mask=mask, maps=hp.read_map('healpy_maps/map_0.fits', field=[1,2], verbose=False, dtype=np.float64), masked_on_input=True ) #beam = b_ell
...
for i in range(nreal):
    ...
    f_mat = nmt.NmtField(mask=mask, maps=[m_pure[0], m_pure[1]], masked_on_input=True) #beam = b_ell
